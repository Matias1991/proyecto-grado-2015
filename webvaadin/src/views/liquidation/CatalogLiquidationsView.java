		...
		dateMonth.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if (dateMonth.getValue() != null) {
					if(dateMonth.isValid() && LiquidationController.existLiquidation(dateMonth.getValue())){						
						dateMonth.setValidationVisible(false);
						cleanTab();						
						cboProject.setEnabled(true);
						txtTypeExchange.setReadOnly(false);
						txtTypeExchange.setConvertedValue(LiquidationController.getTypeExchange(dateMonth.getValue()));
						txtTypeExchange.setReadOnly(true);
						buildComboProjects();
					}else{
						if(dateMonth.isValid())
							new PopupWindow("ERROR", "No existe una liquidación para el mes seleccionado");						
		...
		cboProject.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if (cboProject.getValue() != null && dateMonth.isValid()) {					
					...
					if(selection != -1)
						projectLiquidation = LiquidationController.getProjectLiquidation(dateMonth.getValue(), Integer.parseInt(cboProject.getValue().toString()), RequestContext.getRequestContext().getId());
					else
						companyLiquidation = LiquidationController.getCompanyLiquidation(dateMonth.getValue(), RequestContext.getRequestContext().getId());
					buildTab();
				}else{
					if(!dateMonth.isValid()){
						cboProject.removeAllItems();
