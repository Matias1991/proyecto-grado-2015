			...
			public void buttonClick(ClickEvent event) {
				...
				if (validateEstimateEmployee()) {
					...
					if (aux != null) {
						...
						new PopupWindow("ERROR", "No se pudo estimar");
					}
		...
		btnCreate.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (validateCreateEmployee()) {
					...
					newEmployee.setEmployedType(optEmployeeType.getValue()
							.toString());

					SalarySummary newSalarySummary = new SalarySummary();
					newSalarySummary.setNominalSalary((Double) txtNominalSalary
							.getConvertedValue());
					newSalarySummary.setTickets((Double) txtTickets
							.getConvertedValue());
					if (txtRet.getConvertedValue() == null) {
						txtRet.setConvertedValue(0.0);
					}
					...
					newSalarySummary.setIRPF((Double) txtIrpf
							.getConvertedValue());
					if (txtBse.getConvertedValue() == null) {
						txtBse.setConvertedValue(0.0);
					}
					newSalarySummary.setBSE((Double) txtBse.getConvertedValue());
					newSalarySummary.setHours((Integer) txtHours
							.getConvertedValue());
					if (txtCostSaleHour.getConvertedValue() == null) {
						txtCostSaleHour.setConvertedValue(0.0);
					}
					newSalarySummary.setCostSaleHour((Double) txtCostSaleHour
							.getConvertedValue());
					newSalarySummary
							.setPercentageTypeFONASA((Double) cboPercentagePersonalFonasaContribution
									.getValue() / 100);
