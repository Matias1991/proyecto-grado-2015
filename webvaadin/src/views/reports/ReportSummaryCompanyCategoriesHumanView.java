...
public class ReportSummaryCompanyCategoriesHumanView extends BaseView {
		...
		cboxCategories.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if (cboxCategories.getValue() != null) {
					...
					buildTable(category.getDescription());
				}
			}
		});
	...
	void buildTable(String description)
	{
		if(table != null)
		{
		...
		table = new Table("Custom Table Footer");
		table.addContainerProperty("Proyeto", String.class, null);
		table.addContainerProperty("Importe", Float.class, null);
		table.addContainerProperty("Iva", Integer.class, null);
		table.addContainerProperty("Total", Integer.class, null);
		
		Collection<Category> categories = CategoryController.getCategoriesByDescription(description);
		
		/*
		float totalAmount = 0, totalIva = 0, total = 0;
		int i = 2;
		for(Category category : categories)
		{
			float amount = 0;
			if(category.isCurrencyDollar())		
				amount =(float)category.getAmountDollar();
			else
				amount =(float)category.getAmountPeso();
			
			float iva = 0;
			if(category.getIvaTypeId() == 2)
			{
				iva = (float) (amount * 0.10);
			}
			else if(category.getIvaTypeId() == 2)
			{
				iva = (float) (amount * 0.22);
			}
			...
			table.addItem(new Object [] {category.getProjectName(), (float)amount, (float)iva , (float)amount + iva}, i);
			i++;
		}
		// Set the footers
		table.setFooterVisible(true);
		table.setColumnFooter("Proyeto", "Total");
		table.setColumnFooter("Importe", String.valueOf(totalAmount));
		table.setColumnFooter("Iva", String.valueOf(totalIva));
		table.setColumnFooter("Total", String.valueOf(total));
		
		// Adjust the table height a bit
		table.setPageLength(table.size());
		
		mainLayout.addComponent(table, "top:19%;left:0px;");
		*/
	}
