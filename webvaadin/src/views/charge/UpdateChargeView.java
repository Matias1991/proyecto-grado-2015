...
public class UpdateChargeView extends BaseView {
			...
			public void valueChange(ValueChangeEvent event) {
				if (comboBoxCharges.getValue() != null) {
					...
					if (charge.getIsCurrencyDollar()) {
						...
						txtTypeExchange.setConvertedValue(charge
								.getTypeExchange());
					} else
	...
	public void buildGrid() {
		...
		Collection<Bill> bills = BillController
				.getBillsByFiltersWithChargesAndActiveProjects(
		...
		if (bills != null && bills.size() > 0) {
			...
			billsGrid.setColumnOrder("code", "description",
					"totalAmountToShow", "amountChargedToShow");
	...
	void buildCharges(int billId) {
		...
		for (Charge charge : charges) {
			...
			String caption = String.format("N°: %s - Fecha: %s",
					charge.getNumber(), charge.getCreatedDateTimeUTCToShow());
